...
> **UPDATE — Phase 6 (the "work order") RAN on A100 (2026-06-22). Outcome = NO_REPAIR.** Full results in [`results/A100_run_2026-06-22.md`](results/A100_run_2026-06-22.md). Base 2×2 = **COMPOSE_SPECIFIC** (new no-space `(B*C)=`=0.765 survives). Instruct: all six §7 gates FAIL (C1=0.265 — *worse* than base 0.507; corr(C1)=−0.04) → localization **INVALID**; tuning does **not** install composition. Downstream (all favorable): addition composes (A1=0.995, A2=0.93) but multiplication-compose fails → **operation-specific**; redundant nesting `((0+B))*C`=0.037 → **depth-sensitive**; C1≪C4 at every matched |B·C| bin → magnitude confound killed; **salvage: B decodable at the post-bracket site (CV-R²=0.96) yet C6→C1 patch flip-rate=0.00 → decodable-but-causally-unused.** Framing locked = "composition brittle, operation-specific, depth-sensitive, decodable-but-not-causally-used across base & instruct Llama-3.1-8B." (The Phase 6 harness, `cells/76`–`83`, is CPU-unit-tested in `tests/test_wo_logic.py`; runbook in [`docs/work_order_runbook.md`](docs/work_order_runbook.md).)
